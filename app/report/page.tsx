...
import { SidebarLayout } from "./SidebarNav";
...
export default async function ReportPage() {
  ...
  const [
    ...
    tapHot, tapTest,
    /* newsDigest — 已被 entity_topics 替代 */,
    entityTopics,
  ] = await Promise.all([
    loadSteamWeeklyTopsellersFromSupabase(),
    loadSteamUpcomingPopularFromSupabase(),
  ...
  /* ---------- Footer ---------- */
  const footerNode = (
    <footer className="mt-12 pt-8 border-t-2 border-border-rule pb-16">
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6">
        <div className="space-y-1.5">
          <p className="text-xs text-text-muted">
            数据来源：Steam · Epic Games Store · WeGame · TapTap · 4399 · Bangumi · 国家统计年鉴 · StatCounter
          </p>
          <p className="text-xs text-text-muted">
            生成于{" "}
            <time
  ...
  const sections: Record<string, ReactNode> = {
    /* ── 行业新鲜事 ── */
    "section-news": (
      <IndustryNews data={entityTopics ?? []} />
    ),
      ...
      <div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-border-light border border-border-light">
          <div className="bg-bg-base p-6 sm:p-8 flex flex-col justify-center min-h-[260px]">
            ...
            <div className="hero-number text-5xl tabular-nums">{estPcInUseYi}<span className="text-xl font-normal text-text-secondary ml-1" style={{ fontFamily: "var(--font-body)" }}>亿台</span></div>
          ...
          <div className="bg-bg-base p-6 sm:p-8 min-h-[260px]">
            <SharePie title="PC 出货量份额" subtitle={`${shipmentsPeriodLabel} · 总量 ${pcShipments ? Number(pcShipments.total_million_units).toLocaleString() : "—"}M`} data={sharesRecordToArray(pcShipments?.shares)} labelMap={shipmentsLabelMap} source={pcShipments?.source ? `来源：${pcShipments.note ?? pcShipments.source}` : undefined} />
          </div>
          <div className="bg-bg-base p-6 sm:p-8 min-h-[260px]">
            <SharePie title="PC 搜索引擎份额" subtitle={searchMonthLabel} data={sharesRecordToArray(searchShare?.shares)} labelMap={searchEngineLabelMap} source={searchShare?.source ? `来源：${searchShare.note ?? searchShare.source}` : undefined} />
          </div>
          <div className="bg-bg-base p-6 sm:p-8 min-h-[260px]">
            <SharePie title="PC 浏览器份额" subtitle={browserMonthLabel} data={sharesRecordToArray(browserShare?.shares || {})} labelMap={{ Chrome: "Chrome", Edge: "Edge", "360 Safe": "360浏览器", Safari: "Safari", "QQ Browser": "QQ浏览器", Firefox: "火狐" }} source={browserShare?.source ? `来源：${browserShare.source}` : undefined} />
          </div>
        </div>
      </div>
    ),
                  ...
                  <div className="border-t-2 border-border-rule">
                    <ExpandableList limit={5}>
                      {steamWeeklyEnriched.items.slice(0, 20).map((it, idx) => {
                        ...
                        return (
                          <ChartRow key={`${it.rank}-${it.name ?? it.appid ?? ""}`} rank={it.rank} rankDelta={it.rankDelta} coverUrl={it.headerImage} title={it.name || (it.appid ? `App ${it.appid}` : "—")} titleHref={it.appid ? `https://store.steampowered.com/app/${it.appid}/` : null} titleExtra={it.weeksOnChart === 1 ? <NewTag /> : undefined} subtitle={subtitle} staggerIndex={idx} priceMain={isFreeText(it.priceText) ? <FreePill /> : (it.priceText || "—")} priceExtra={typeof it.discountPercent === "number" && it.discountPercent > 0 ? <DiscountTag percent={it.discountPercent} /> : undefined} />
                        );
                      })}
                    </ExpandableList>
                  </div>
                      ...
                      {steamWeeklyEnriched.moversUp.length === 0 ? (
                        ...
                        <ExpandableList limit={5}>
                          {steamWeeklyEnriched.moversUp.map((it, idx) => (
                            <ChartRow key={`up-${it.appid ?? it.rank}`} rank={it.rank} rankDelta={it.rankDelta} coverUrl={it.headerImage} title={it.name || (it.appid ? `App ${it.appid}` : "—")} titleHref={it.appid ? `https://store.steampowered.com/app/${it.appid}/` : null} subtitle={[it.genres.length ? it.genres.join(" · ") : "类型未知", typeof it.weeksOnChart === "number" ? `在榜 ${it.weeksOnChart} 周` : null].filter(Boolean).join(" · ")} staggerIndex={idx} priceMain={isFreeText(it.priceText) ? <FreePill /> : (it.priceText || "—")} priceExtra={typeof it.discountPercent === "number" && it.discountPercent > 0 ? <DiscountTag percent={it.discountPercent} /> : undefined} compact />
                          ))}
                        </ExpandableList>
                      )}
                    </div>
                      ...
                      {steamWeeklyEnriched.newOnChart.length === 0 ? (
                        ...
                        <ExpandableList limit={5}>
                          {steamWeeklyEnriched.newOnChart.map((it, idx) => (
                            <ChartRow key={`new-${it.appid ?? it.rank}`} rank={it.rank} rankDelta={it.rankDelta} coverUrl={it.headerImage} title={it.name || (it.appid ? `App ${it.appid}` : "—")} titleHref={it.appid ? `https://store.steampowered.com/app/${it.appid}/` : null} titleExtra={<NewTag />} subtitle={[it.genres.length ? it.genres.join(" · ") : "类型未知", typeof it.weeksOnChart === "number" ? `在榜 ${it.weeksOnChart} 周` : null].filter(Boolean).join(" · ")} staggerIndex={idx} priceMain={isFreeText(it.priceText) ? <FreePill /> : (it.priceText || "—")} priceExtra={typeof it.discountPercent === "number" && it.discountPercent > 0 ? <DiscountTag percent={it.discountPercent} /> : undefined} compact />
                          ))}
                        </ExpandableList>
                      )}
                    </div>
          ...
          <div>
            ...
            {steamUpdatesSummary ? (
              <div className="max-w-3xl">
                {steamUpdatesSummary.title ? <h4 className="text-base font-medium text-text-primary mb-1">{steamUpdatesSummary.title}</h4> : null}
                {steamUpdatesSummary.updatedAt ? <p className="text-xs text-text-muted mb-4">更新：{dateOnlyLabel(steamUpdatesSummary.updatedAt)}</p> : null}
                <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-wrap break-words">{steamUpdatesSummary.body}</p>
                {steamUpdatesSummary.extra ? <a href={steamUpdatesSummary.extra} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-text-accent mt-4 hover:underline">查看详情 <span className="link-arrow">→</span></a> : null}
              </div>
            ) : (
              <div className="py-10 text-center text-sm text-text-muted">暂无数据</div>
            )}
            ...
            {steamUpcomingEnriched?.items?.length ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-border-light border border-border-light">
                {steamUpcomingEnriched.items.slice(0, 18).map((it, idx) => (
                  <div key={`${it.rank}-${it.name}`} className="flex gap-3 p-4 bg-bg-base group animate-card-in" style={{ "--stagger": idx } as React.CSSProperties}>
                    <div className="w-14 h-14 flex-shrink-0 overflow-hidden bg-bg-surface" style={{ borderRadius: "2px" }}>
                      {it.headerImage ? <img src={it.headerImage} alt={it.name} className="object-cover w-full h-full cover-zoom" loading="lazy" referrerPolicy="no-referrer" /> : null}
                    </div>
                    <div className="min-w-0 flex-1">
                      <ExternalLink href={it.appid ? `https://store.steampowered.com/app/${it.appid}/` : null} className="text-sm font-medium text-text-primary truncate block hover:text-text-accent transition-colors">{it.name}</ExternalLink>
                      {it.genres.length > 0 ? <div className="flex gap-1 mt-0.5 overflow-hidden">{it.genres.slice(0, 3).map((t) => <span key={t} className="inline-block px-1.5 py-px text-[10px] leading-tight rounded bg-bg-surface text-text-muted truncate max-w-[80px]">{t}</span>)}</div> : null}
                      <div className="mt-1.5 flex items-center gap-3 text-xs text-text-muted">
                        <span>{it.releaseDateText || "日期待定"}</span>
                        {typeof it.followers === "number" ? <span className="font-mono tabular-nums">{(it.followers / 1000).toFixed(1)}K 关注</span> : null}
                      </div>
                    </div>
                  </div>
                ))}
                {steamUpcomingEnriched.items.slice(0, 18).length % 2 !== 0 && <div className="hidden sm:block bg-bg-base" />}
              </div>
            ) : (
              <div className="py-10 text-center text-sm text-text-muted">暂无数据</div>
            )}
          ...
          <div>
            ...
            {steamMonthlyNewEnriched?.items?.length ? (
              <div className="grid gap-8 sm:grid-cols-2">
                {(["gold", "silver"] as const).filter((tier) => steamMonthlyNewEnriched.items.some((x) => x.tier === tier)).map((tier) => {
                  ...
                  return (
                    <div key={tier}>
                      ...
                      <div className="border-t-2 border-border-rule">
                        <ExpandableList limit={5}>
                          {list.slice(0, 12).map((it, idx) => <ChartRow key={`${tier}-${it.name}`} rank={idx + 1} coverUrl={it.headerImage} title={it.name} titleHref={it.appid ? `https://store.steampowered.com/app/${it.appid}/` : null} subtitle={it.genres.length ? it.genres.join(" · ") : "类型未知"} staggerIndex={idx} priceMain={isFreeText(it.priceText) ? <FreePill /> : (it.priceText || "—")} priceExtra={typeof it.discountPercent === "number" && it.discountPercent > 0 ? <DiscountTag percent={it.discountPercent} /> : undefined} />)}
                        </ExpandableList>
                      </div>
                    </div>
                  );
              ...
              {epicCharts?.topSellers?.length ? (
                <ExpandableList limit={5}>
                  {epicCharts.topSellers.slice(0, 10).map((g, idx) => {
                    ...
                    const priceMain = g.is_free === true ? <FreePill /> : hasDiscount && current && original ? (<div className="flex items-center justify-end gap-1.5"><span className="text-xs text-text-muted line-through tabular-nums">{original}</span><span className="text-sm font-medium text-text-primary tabular-nums">{current}</span></div>) : current ?? "—";
                    return <ChartRow key={`epic-top-${g.rank}`} rank={g.rank} coverUrl={g.cover_image} title={g.name} titleHref={g.epic_store_url} subtitle={subtitle} staggerIndex={idx} priceMain={priceMain} priceExtra={hasDiscount ? <DiscountTag percent={g.discount_percent!} /> : undefined} />;
                  })}
                </ExpandableList>
              ) : (
              ...
              {epicCharts?.mostPlayed?.length ? (
                <ExpandableList limit={5}>
                  {epicCharts.mostPlayed.slice(0, 10).map((g, idx) => {
                    ...
                    const current = g.current_price_num != null ? formatMoney(g.current_price_num, g.currency) : g.current_price_usd != null ? formatMoney(g.current_price_usd, "USD") : null;
                    const original = g.original_price_num != null ? formatMoney(g.original_price_num, g.currency) : g.original_price_usd != null ? formatMoney(g.original_price_usd, "USD") : null;
                    const priceMain = isFree ? <FreePill /> : hasDiscount && current && original ? (<div className="flex items-center justify-end gap-1.5"><span className="text-xs text-text-muted line-through tabular-nums">{original}</span><span className="text-sm font-medium text-text-primary tabular-nums">{current}</span></div>) : current ?? "—";
                    return <ChartRow key={`epic-mp-${g.rank}`} rank={g.rank} coverUrl={g.cover_image} title={g.name} titleHref={g.epic_store_url} subtitle={subtitle} staggerIndex={idx} priceMain={priceMain} priceExtra={hasDiscount ? <DiscountTag percent={g.discount_percent!} /> : undefined} />;
                  })}
                </ExpandableList>
              ) : (
                <div className="py-6 text-sm text-text-muted text-center">暂无数据</div>
          ...
          {epicFree?.games?.length ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-border-light border border-border-light">
              {epicFree.games.slice(0, 20).map((g, idx) => {
                const dateText = g.startAt && g.endAt ? `${dateOnlyLabel(g.startAt)} 至 ${dateOnlyLabel(g.endAt)}` : g.startAt ? `${dateOnlyLabel(g.startAt)} 起` : g.endAt ? `截止 ${dateOnlyLabel(g.endAt)}` : null;
                return (
                  <div key={`epic-free-${g.rank}-${g.name}`} className="flex items-start gap-3 p-4 bg-bg-base group animate-card-in" style={{ "--stagger": idx } as React.CSSProperties}>
                    <div className="w-14 h-14 flex-shrink-0 overflow-hidden bg-bg-surface" style={{ borderRadius: "2px" }}>{g.cover_image ? <img src={normalizeCoverUrl(g.cover_image) ?? undefined} alt={g.name} className="object-cover w-full h-full cover-zoom" loading="lazy" referrerPolicy="no-referrer" /> : null}</div>
                    <div className="min-w-0 flex-1">
                      <ExternalLink href={g.epic_store_url} className="text-sm font-medium text-text-primary truncate block hover:text-text-accent transition-colors">{g.name}</ExternalLink>
                      {g.tags.length > 0 ? <div className="flex gap-1 mt-0.5 overflow-hidden">{g.tags.slice(0, 3).map((t) => <span key={t} className="inline-block px-1.5 py-px text-[10px] leading-tight rounded bg-bg-surface text-text-muted truncate max-w-[80px]">{t}</span>)}</div> : null}
                      <p className="text-xs text-text-muted truncate mt-0.5">{dateText || "—"}</p>
                    </div>
                  </div>
                );
              })}
              {(() => { const remainder = epicFree.games.slice(0, 20).length % 3; if (remainder === 0) return null; const fillers = 3 - remainder; return Array.from({ length: fillers }, (_, i) => <div key={`epic-free-filler-${i}`} className="hidden sm:block bg-bg-base" />); })()}
            </div>
          ) : (
            <div className="py-6 text-sm text-text-muted text-center">暂无数据</div>
          )}
    ...
    "section-wegame": (
      <div>
        <WeGameSection bestseller={wgBestseller} purchase={wgPurchase} follow={wgFollow} inline />
      </div>
    ),
                  ...
                  <ExpandableList limit={5}>
                    {x.pack.games.slice(0, 20).map((g, idx) => {
                      ...
                      if (g.tags.length) {
                        ...
                        const cleanedTags = g.tags.map((t) => t.trim()).filter(Boolean).filter((t) => { if (x.key !== "hot") return true; if (norm(t) === "免费下载") return false; if ((g.price ?? "").trim() && norm(t) === norm(g.price ?? "")) return false; return true; }).slice(0, 3);
                        if (cleanedTags.length) sub.push(cleanedTags.join(" · "));
                      }
                      return <ChartRow key={`${x.key}-${g.rank}`} rank={g.rank} coverUrl={g.cover_image} title={g.title} titleHref={g.store_url} subtitle={sub.length ? sub.join(" · ") : "—"} staggerIndex={idx} priceMain={x.key === "test" ? g.test_status ?? "—" : isFreeText(g.price) ? <FreePill /> : g.price ?? "—"} />;
                    })}
                  </ExpandableList>
                ) : (
                  <div className="py-6 text-sm text-text-muted text-center">暂无数据</div>
      ...
      <div>
        {summary4399New ? (
          <div>
            <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-xs text-text-muted">{summary4399New.timeWindow ?? "—"}{summary4399New.updatedAt ? ` · 更新 ${dateOnlyLabel(summary4399New.updatedAt)}` : ""}</p>
              </div>
              <div className="text-right">
                <div className="hero-number text-3xl tabular-nums">{typeof summary4399New.totalCount === "number" ? summary4399New.totalCount.toLocaleString() : "—"}</div>
            ...
            {Object.keys(summary4399New.categoryBreakdown).length ? (
              <div>
                ...
                <div className="space-y-2">
                  {(() => {
                    const entries = Object.entries(summary4399New.categoryBreakdown).filter(([, v]) => typeof v === "number" && Number.isFinite(v) && v > 0).sort((a, b) => b[1] - a[1]).slice(0, 10);
                    const total = entries.reduce((acc, [, v]) => acc + v, 0);
                    return entries.map(([k, v], idx) => {
                      const pct = total > 0 ? Math.round((v / total) * 100) : 0;
                      return (
                        <div key={k} className="flex items-center gap-3 animate-row-in" style={{ "--stagger": idx } as React.CSSProperties}>
                          <div className="w-16 shrink-0 truncate text-xs text-text-secondary text-right" title={k}>{k}</div>
                          <div className="flex-1 h-4 bg-bg-surface overflow-hidden" style={{ borderRadius: "1px" }}><div className="h-full animate-bar-grow" style={{ width: `${pct}%`, backgroundColor: "var(--color-4399)", opacity: 0.6, "--stagger": idx } as React.CSSProperties} /></div>
                          <div className="w-16 shrink-0 text-right font-mono text-xs tabular-nums text-text-muted">{v} <span className="text-text-muted/60">({pct}%)</span></div>
                        </div>
                      );
                    });
                  })()}
          ...
          <div className="max-w-3xl">
            ...
            {summary4399.updatedAt ? <p className="mt-1 text-xs text-text-muted">更新：{dateOnlyLabel(summary4399.updatedAt)}</p> : null}
            <pre className="mt-4 max-h-[420px] overflow-auto whitespace-pre-wrap break-words text-sm leading-relaxed text-text-secondary">{summary4399.body}</pre>
            {summary4399.extra ? <a className="mt-3 inline-flex text-xs text-text-accent hover:underline" href={summary4399.extra} target="_blank" rel="noreferrer">查看详情 →</a> : null}
          </div>
        ) : (
          <div className="py-10 text-center text-sm text-text-muted">暂无数据</div>
        )}
